...
int16_t shoot_step = 10;
int16_t shoot_temp[2];
static void shoot_ramp(void)
{
	for(uint8_t i=0; i<2; i++)
	{
		if(ABS(shoot.fric_spd_ref[i] - shoot_temp[i]) > shoot_step) 
		{	
			if(shoot.fric_spd_ref[i] > shoot_temp[i])
				shoot_temp[i] += shoot_step;
			else
				shoot_temp[i] -= shoot_step;
		}else{
			shoot_temp[i] = shoot.fric_spd_ref[i];
		}		
	}
}
...
{
	OUTPUT_LIMIT(shoot.fric_spd_ref[R], 9000, 0);
	OUTPUT_LIMIT(shoot.fric_spd_ref[L], 0, -9000);
	shoot_ramp();
	shoot.fric_current[R] = PID_Calculate(&fric_motor_pid_t[R], shoot.fric_spd_fbd[R], shoot_temp[R]);
	shoot.fric_current[L] = PID_Calculate(&fric_motor_pid_t[L], shoot.fric_spd_fbd[L], shoot_temp[L]);
