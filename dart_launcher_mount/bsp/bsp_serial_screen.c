...
int32_t serial_gimbal_vision_pos[4];/*= {-200000,-205000,-210000,-220000}*/
uint8_t serial_num;
uint8_t serial_protect_flag;
uint8_t flash_flag;
uint32_t flash_temp[10];
...
void Serial_Screen_Task(const void* argu)
{
	...
	memcpy(serial_gimbal_vision_pos, (uint32_t*)GIMBAL_VISION_ADDR, sizeof(serial_gimbal_vision_pos));
...
//	
//	//等镖架初始化完了，串口屏再跳界面
//	if(SBUS.SB == SW_UP)//遥控模式，云台数值为实际total_ecd
//	{
//		//跳出初始化界面
//		Uart_printf(&SERIAL_SCREEN_HUART, "start.j0.val=100\xff\xff\xff");
////		//初始化test界面的值
...
////		//初始化test界面的值
////		Uart_printf(&SERIAL_SCREEN_HUART, "test.n0.val=%d\xff\xff\xff", serial_fric_vel[3]);
////		Uart_printf(&SERIAL_SCREEN_HUART, "test.n1.val=%d\xff\xff\xff", -serial_gimbal_vision_pos[2]);
////		Uart_printf(&SERIAL_SCREEN_HUART, "test.n2.val=3\xff\xff\xff");
////		
////		//初始化game模式的值
////		Uart_printf(&SERIAL_SCREEN_HUART, "game.n2.val=0\xff\xff\xff");
////		Uart_printf(&SERIAL_SCREEN_HUART, "game.n0.val=%d\xff\xff\xff", serial_fric_vel[0]);
////		Uart_printf(&SERIAL_SCREEN_HUART, "game.n1.val=%d\xff\xff\xff", -serial_gimbal_vision_pos[0]);
//	}	
...
static void Screen_Test_Handle(uint8_t* screen_buf)
{
	switch (screen_buf[1])
	{
		...
		case 't':				//串口屏发送数据更新单片机数据  先发低八位
		{
			...
			if(SBUS.SB == SW_DN)
				serial_gimbal_vision_pos[2] = -(screen_buf[5] | screen_buf[6]<<8 | screen_buf[7]<<16 | screen_buf[8]<<24);				
			else //if(SBUS.SB == SW_UP||SW_MI)
...
static void Screen_Test_Mode(void)
{
	if(gimbal_remote_change_flag)//遥控模式才会置次标志位
	{
		Uart_printf(&SERIAL_SCREEN_HUART, "test.n1.val=%d\xff\xff\xff", -serial_gimbal_pos[2]);	
		gimbal_remote_change_flag = 0;
	}
	if(screen_update_flag)//在串口屏中点击“更新数据”，要求更新对应模式的数据
	{
		if(SBUS.SB == SW_DN)
		{		
			Uart_printf(&SERIAL_SCREEN_HUART, "test.n0.val=%d\xff\xff\xfftest.n1.val=%d\xff\xff\xff", 	\
												serial_fric_vel[3], -serial_gimbal_vision_pos[2]);
		}
		else //if(SBUS.SB == SW_UP ||SW_MI)//仅需根据遥控器模式(REMOTE/VISION)提供不同模式下数组的数据
		{
			Uart_printf(&SERIAL_SCREEN_HUART, "test.n0.val=%d\xff\xff\xfftest.n1.val=%d\xff\xff\xff",	\
												serial_fric_vel[3], -serial_gimbal_pos[2]);		
		}
...
static void Screen_Game_Handle(uint8_t* screen_buf)
{
	switch (screen_buf[1])
	{
		...
		case 't':				//串口屏发送数据更新单片机数据  先发低八位
		{
			...
			if(SBUS.SB == SW_DN)
				serial_gimbal_vision_pos[screen_buf[2]] = -(screen_buf[5] | screen_buf[6]<<8 | screen_buf[7]<<16 | screen_buf[8]<<24);				
			else //if(SBUS.SB == SW_UP||SW_MI)			
...
static void Screen_Game_Mode(void)
{
	if(flash_flag)//保存串口屏数据进flash
	{
		...
		memcpy(flash_temp+6, serial_gimbal_vision_pos, 16);
		WriteFlashData(FRIC_ADDR, flash_temp, 10);
		flash_flag = 0;
	}
	if(screen_update_flag)//在串口屏中点击“镖体编号”，要求更新对应编号的数据
	{
		if(SBUS.SB == SW_DN)
		{
			Uart_printf(&SERIAL_SCREEN_HUART, "game.n0.val=%d\xff\xff\xffgame.n1.val=%d\xff\xff\xffgame.n2.val=%d\xff\xff\xff"\
												,serial_fric_vel[serial_num], -serial_gimbal_vision_pos[serial_num], serial_num);
		}
		else //if(SBUS.SB == SW_UP||SW_UP)//仅需根据遥控器模式(REMOTE/VISION)提供不同模式下数组的数据
		{
			Uart_printf(&SERIAL_SCREEN_HUART, "game.n0.val=%d\xff\xff\xffgame.n1.val=%d\xff\xff\xffgame.n2.val=%d\xff\xff\xff"\
											, serial_fric_vel[serial_num], -serial_gimbal_pos[serial_num], serial_num);
		}
