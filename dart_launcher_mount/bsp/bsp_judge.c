	...
	if(ReadFromUsart[ SOF ] == JUDGE_FRAME_HEADER)
	{
		...
		if (Verify_CRC8_Check_Sum( ReadFromUsart, LEN_HEADER ) == TRUE)
		{
			...
			if(Verify_CRC16_Check_Sum(ReadFromUsart,judge_length) == TRUE)
			{
				...
				switch(CmdID)
				{
					case ID_game_state:     //0x0001  比赛状态数据  1Hz
					...
					case ID_event_data:    	 //0x0101    场地事件数据  1Hz
						memcpy(&Event_Data, (ReadFromUsart + DATA), LEN_event_data);
					break;
										
					case ID_judge_warning:  	//0x0104  裁判警告数据  1Hz
						memcpy(&Referee_Warning, (ReadFromUsart + DATA), LEN_judge_warning);
					break;
					
					case ID_dart_info:  //0x0105  飞镖发射相关数据 1Hz
					...
					case ID_dart_client:      		//0x020A  飞镖选手端指令数据，固定以3Hz发送
